...
from oemer.utils import save_intermediate
...
def extract(args: Namespace) -> str:
    ...
    # Optionally save intermediate predictions before dewarp
    if getattr(args, 'save_intermediate', False):
        out_dir = os.path.join(args.output_path, f"{f_name}_intermediate")
        artifacts = {
            'original_image': image,
            'staff_pred_raw': (staff * 255).astype(np.uint8),
            'symbols_pred_raw': (symbols * 255).astype(np.uint8),
            'stems_rests_pred_raw': (stems_rests * 255).astype(np.uint8),
            'notehead_pred_raw': (notehead * 255).astype(np.uint8),
            'clefs_keys_pred_raw': (clefs_keys * 255).astype(np.uint8),
        }
        save_intermediate(out_dir, artifacts)
    ...
    if not args.without_deskew:
        ...
        # Save dewarped images if requested
        if getattr(args, 'save_intermediate', False):
            out_dir = os.path.join(args.output_path, f"{f_name}_intermediate")
            artifacts = {
                'dewarped_image': image,
                'staff_pred_dewarped': (staff * 255).astype(np.uint8),
                'symbols_pred_dewarped': (symbols * 255).astype(np.uint8),
                'stems_rests_pred_dewarped': (stems_rests * 255).astype(np.uint8),
                'notehead_pred_dewarped': (notehead * 255).astype(np.uint8),
                'clefs_keys_pred_dewarped': (clefs_keys * 255).astype(np.uint8),
            }
            save_intermediate(out_dir, artifacts)
