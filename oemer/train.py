...
class MultiprocessingDataLoader:
    def __init__(self, num_worker: int):
        # Windows spawn mode can fail to pickle this loader object due to
        # bound-method process targets. Fallback to single-process generation.
        self._use_multiprocessing = os.name != "nt"
        self._queue = None
        self._dist_queue = None
        self._process_pool = []
        self._pdist = None
...
class DataLoader(MultiprocessingDataLoader):
    ...
    def _load_one_sample(self):
        inp_img_path, staff_img_path, symbol_img_path = self.feature_files[self.file_idx]
        self.file_idx += 1
        if self.file_idx == len(self.feature_files):
            random.shuffle(self.feature_files)
            self.file_idx = 0
        ...
        np.float = float  # Monkey patch to workaround removal of np.float
        perspect_trans = lambda img: imaugs.perspective_transform(img, seed=seed, sigma=70)
        image = np.array(perspect_trans(image))
        staff_img = np.array(perspect_trans(staff_img))
        symbol_img = np.array(perspect_trans(symbol_img))
        staff_img = np.where(staff_img, 1, 0)
        symbol_img = np.where(symbol_img, 1, 0)

        return image, staff_img, symbol_img, ratio
    ...
    def __iter__(self):
        ...
        while samples < self.num_samples:
            if self._use_multiprocessing:
                ...
                image, staff_img, symbol_img, ratio = self._load_one_sample()
...
class DsDataLoader(MultiprocessingDataLoader):
    ...
    def _load_one_sample(self):
        inp_img_path, seg_img_path = self.feature_files[self.file_idx]
        self.file_idx += 1
        if self.file_idx == len(self.feature_files):
            random.shuffle(self.feature_files)
            self.file_idx = 0
        ...
        np.float = float  # Monkey patch to workaround removal of np.float
        perspect_trans = lambda img: imaugs.perspective_transform(img, seed=seed, sigma=70)
        image = np.array(batch_transform(image, perspect_trans))
        label = np.array(batch_transform(label, perspect_trans))
    ...
    def __iter__(self):
        ...
        while samples < self.num_samples:
            if self._use_multiprocessing:
                ...
                image, label, ratio = self._load_one_sample()
...
def focal_tversky_loss(y_true, y_pred, fw=0.7, alpha=0.7, smooth=1., gamma=0.75):
    ...
    focal_loss = tf.keras.losses.BinaryFocalCrossentropy(from_logits=False)(y_true, y_pred)
