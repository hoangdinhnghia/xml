...
## 🆕 Thesis Enhancements (May 2026)

**For thesis submission**: This repository has been enhanced with improvements for building an OMR system using U-Net segmentation.

### ✨ 5 Key Improvements Made

1. **Inference Pipeline Refactoring** ([inference.py](oemer/inference.py))
   - Split monolithic function into 4 modular functions: `load_model()`, `tile_image()`, `predict_patches()`, `merge_patches()`
   - Enables independent testing and optimization of each stage
   - Backward compatible with original API

2. **Intermediate Artifact Saving** ([ete.py](oemer/ete.py))
   - New CLI flag `--save-intermediate` to save segmentation masks at pipeline stages
   - Useful for debugging and visualizing intermediate predictions
   - Stored in `<output>/basename_intermediate/`

3. **Comprehensive Evaluation Framework** ([eval/](eval/))
   - **Pixel-level segmentation metrics**: IoU, Precision, Recall, F1
   - **Note-level MusicXML evaluation**: Onset-based matching using music21
   - Both available via CLI and Python API

4. **Improved Notehead Extraction** ([notehead_extraction.py](oemer/notehead_extraction.py))
   - Replaced simple center-split with watershed segmentation
   - Better handling of touching/overlapping noteheads
   - Fallback to center-split for edge cases

5. **Automated Benchmark System** ([bench/](bench/))
   - Orchestrates full pipeline: inference → evaluation → report generation
   - Generates CSV metrics and summary REPORT.txt
...
│   └── oemer/            # Main OMR package
├── data/                 # All data organized
│   ├── ground_truth/     # MusicXML reference (auto-converted from JSON)
│   ├── images/           # Test images
│   └── datasets/         # Reference datasets (CvcMuscima, ds2_dense)
├── scripts/              # Utility scripts (convert, train, etc.)
├── eval/                 # Segmentation + end-to-end evaluation
├── bench/                # Benchmark automation
├── tests/                # Unit tests
└── docs/                 # Documentation & results
```

📚 **Full Documentation**: 
- [QUICKREF.md](QUICKREF.md) - Quick reference & command list
- [STRUCTURE.md](STRUCTURE.md) - Detailed repository structure
- [MIGRATION_GUIDE.md](MIGRATION_GUIDE.md) - Restructuring instructions
